    ...
    function createPath(strength) {
        ...
        path.data = {};
        path.data.springs = [];
        //var sub = '#'+(0x1000000+(Math.random())*0xffffff).toString(16).substr(1,6)
        path.fillColor = "#000";
    ...
    function onMouseMove(event) {
        ...
        if (!point.fixed && y + range > point.y && y - range < point.y) {
            point.y += (y - point.y) / 6;
            if (prev && !prev.fixed) prev.y += (y - prev.y) / 6;
            if (next && !next.fixed) next.y += (y - next.y) / 6;
        }
    }
